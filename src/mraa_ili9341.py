...
def test_fullscreen_color_paint(lcd):
    for name, color in list(TEST_RGB_COLORS.items())[:4]:
        print("Painting color {} in fullscreen ...".format(name))
        lcd.framebuff[:, :, :] = color
        print(lcd.framebuff.shape)
        lcd.update()
        time.sleep(2.0)


def test_rectangular_boxes(lcd, size=100):
...
def test_run_moon_landing(lcd):
    from PIL import Image
    import cv2

    lcd.clear()

    PATH_TEMPLATE = "./moon_landing/frame{:04d}.jpg"
    for i in range(1, 1476 + 1):
...
def test_display_image(lcd):
   from PIL import Image
   import cv2

   lcd.clear() 
   img = cv2.cvtColor(cv2.imread("dandy.png"), cv2.COLOR_BGR2RGB)
   img = cv2.cvtColor(cv2.imread("cat_small.png"), cv2.COLOR_BGR2RGB)
   ...
   for _ in range(20):
       # lcd.framebuff[:48, :48, :] = img[:48, :48, :]
       lcd.framebuff[:, :, :] = 0
       # lcd.framebuff[:200, :200, :] = img[:200, :200, :]
       lcd.framebuff[:, :, :] = img
       lcd.update()
       time.sleep(1.0)
...
def test_webcam(lcd):
    ...
    while True:
        ret, frame = cap.read()
        frame = cv2.cvtColor(cv2.resize(frame, (100, 100)), cv2.COLOR_BGR2RGB)
        lcd.framebuff[:100, :100, :] = frame
        lcd.update()
        time.sleep(1 / 30)
...
if __name__ == "__main__":
    ...
    # i = 0
    # while True:
    #     lcd.framebuff[0:100, 0:200, :] = (0x00, i % 256, 0)
    #     lcd.update()
    #     i += 1
    #     time.sleep(0.1)
    ...
    # test_fullscreen_color_paint(lcd)
    # time.sleep(1.0)
    # test_rectangular_boxes(lcd)

    # import sys
    # sys.exit(0)


    # lcd.framebuff[0:50, 0:50, 0] = 255
    # lcd.framebuff[0:50, 0:50, 1] = 0
    # lcd.framebuff[0:50, 0:50, 2] = 0
    ...
    for i in range(1000000):
        ...
        if i % 10 == 0:
            top = random.randint(0, 239 - 50)
            left = random.randint(0, 319 - 50)
            bot = top + 100
            right = left + 100
